...
| `ntfy-credentials` | `token` | Bearer token for the `hermes` user on `ntfy.jannekeipert.de`, scoped write-only to `hermes-mail` (house pattern: one user per publisher). Recreate with `ntfy user add hermes`, `ntfy access hermes hermes-mail write`, `ntfy token add hermes` inside the ntfy pod. |
...
| `ntfy-credentials` | ✅ real token sealed — ntfy user `hermes`, write-only on `hermes-mail` |
